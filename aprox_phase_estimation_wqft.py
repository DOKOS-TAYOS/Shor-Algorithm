...
Author: Gonzalo Ferro Costas & Alberto Manzano Herrero, QFT by Alejandro Mata Ali
...
from QQuantLib.PE.Aprox_Shor import QFT_aprox

class PhaseEstimationwQFTaprox:
    """
    ...
    def __init__(self, **kwargs):
        ...
        kwars : dictionary
            ...
            shots : int
                ...
                will be computed.
            aprox : int
                number of interactions in the QFT aproximation
        """
    ...
    def pe_wqft(self, number_of_cbits=None, shots=None):
        ...
        self.init_pe()
        self.q_prog = self.apply_pe_wqft(self.q_prog, self.q_gate, self.q_aux, self.aprox)
    ...
    @staticmethod
    def apply_inv_qft(q_prog_, q_aux, aprox):
        """
        ...
        """
        q_prog = deepcopy(q_prog_)
        q_prog.apply(QFT_aprox(len(q_aux), aprox).dag(), q_aux)
        return q_prog

    @staticmethod
    def apply_pe_wqft(q_prog_, q_gate, q_aux, aprox):
        """
        ...
        """
        q_prog = deepcopy(q_prog_)
        q_prog = PhaseEstimationwQFTaprox.apply_controlled_operations(q_prog, q_gate, q_aux)
        q_prog = PhaseEstimationwQFTaprox.apply_inv_qft(q_prog, q_aux, aprox)
        return q_prog
